...
public class ShopItem : MonoBehaviour
{
  ...
  //타일 박스 콜라이더로 지정..
    ...
    Test_WRandomPick RandomRelic;
    Test_WRandomPick1 RandomChart;
    ...
    { 
        ...
        */
        ...
        RandomRelic = GetComponent<Test_WRandomPick>();
        RandomChart = GetComponent<Test_WRandomPick1>();
        //RandomRelic.relics_random=new List<string>();
        //RandomRelic1.relics_random=new List<string>();
        RandomRelic.Roll();
        RandomChart.Roll();
    ...
    public void Transforming_music(){
        foreach (GameObject j in music_note){
            if(j.name==RandomChart.relics_random[0]){
                GameObject instantiatedrelics = Instantiate(j);
                instantiatedrelics.transform.position=new Vector3(36,13,0);
             }
        }
        foreach (GameObject j in music_note){
            if(j.name==RandomChart.relics_random[1]){
                GameObject instantiatedrelics = Instantiate(j);
